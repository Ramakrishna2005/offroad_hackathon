...
def evaluate_metrics(model, backbone, data_loader, device, num_classes=10, show_progress=True):
    """Evaluate all metrics on a dataset, returning per-class averages as well."""
    iou_scores = []
    dice_scores = []
    pixel_accuracies = []
    all_class_iou = []  # NEW: list to hold per-class IoU across batches
    ...
    with torch.no_grad():
        for imgs, labels in loader:
            ...
            dice, class_dice = compute_dice(outputs, labels, num_classes=num_classes)
            pixel_acc = compute_pixel_accuracy(outputs, labels)

            iou_scores.append(iou)
            dice_scores.append(dice)
            pixel_accuracies.append(pixel_acc)
            all_class_iou.append(class_iou) # Store batch's class IoU

    model.train()
    
    # Average the per-class IoU across all batches
...
def main():
    ...
    transform = transforms.Compose([
        ...
        transforms.ColorJitter(brightness=0.3, contrast=0.3, saturation=0.3, hue=0.1), # Added this line
        transforms.ToTensor(),
    ...
    # --- YOU ACCIDENTALLY DELETED THIS BLOCK ---
    ...
    # Create a custom function to combine them
    def combined_loss(outputs, labels):
        return ce_loss(outputs, labels) + dice_loss(outputs, labels)
        
    loss_fct = combined_loss
    
    # Upgrade optimizer to AdamW for the Transformer/ConvNeXt architecture
    ...
    for epoch in epoch_pbar:
        ...
        # Step the learning rate scheduler at the end of the train epoch
        scheduler.step()
    ...
    print("\nFinal Per-Class Validation IoU:")
