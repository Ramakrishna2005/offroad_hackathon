...
# Get script directory for default paths
    ...
    # 1. Point to the weights you just trained!
    parser.add_argument('--model_path', type=str, default='/kaggle/working/segmentation_head.pth',
                        help='Path to trained model weights')
    
    # 2. Point to the Kaggle Validation folder
    parser.add_argument('--data_dir', type=str, default='/kaggle/input/datasets/pudathukrishna/train-again/Offroad_Segmentation_Training_Dataset/val',
                        help='Path to validation dataset')
    
    # 3. Save the output images directly to Kaggle's working directory
    parser.add_argument('--output_dir', type=str, default='/kaggle/working/final_predictions',
    ...
    # NEW: Strip the "module." prefix from the dual-GPU weights
    ...
    classifier.load_state_dict(clean_state_dict)
    ...
    with torch.no_grad():
        ...
        for batch_idx, (imgs, labels, data_ids) in enumerate(pbar):
            ...
            # --- TEST-TIME AUGMENTATION (TTA) FORWARD PASS ---
            
            # 1. Original Image Prediction
            features_orig = backbone_model.forward_features(imgs)["x_norm_patchtokens"]
            logits_orig = classifier(features_orig.to(device))
            outputs_orig = F.interpolate(logits_orig, size=imgs.shape[2:], mode="bilinear", align_corners=False)
            
            # 2. Flipped Image Prediction
            # Flip the batch of images horizontally (dim=-1 is the width dimension)
            imgs_flipped = torch.flip(imgs, dims=[-1])
            features_flipped = backbone_model.forward_features(imgs_flipped)["x_norm_patchtokens"]
            logits_flipped = classifier(features_flipped.to(device))
            outputs_flipped = F.interpolate(logits_flipped, size=imgs_flipped.shape[2:], mode="bilinear", align_corners=False)
            
            # Un-flip the predicted masks so they align with the original image
            outputs_flipped_rev = torch.flip(outputs_flipped, dims=[-1])
            
            # 3. Average the Predictions for maximum confidence
            outputs = (outputs_orig + outputs_flipped_rev) / 2.0
